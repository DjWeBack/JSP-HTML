...
<!-- 공통코드 영역 -->
	...
	<script type="text/javascript">
		...
		function cut(){
			...
			/* $("#dg_member").datagrid({'selectOnCheck':$(this).is(':checked')});
			alert(row);
			alert(row.mem_id); */
 			var index=$("#dg_member").datagrid('getRowIndex', row);
			delRow(index); 
			//alert("삭제 호출 성공");
/* 			$("#dg_member").datagrid({'selectOnCheck':$(this).is(':checked'),onchange:function(){
				alert('test');
			}});
			$("#dg_member").datagrid({'checkOnSelect':$(this).is(':checked'),onchange:function(){
				alert('test2');
			}}); */
			//var row = $("#dg_member").datagrid('getSelected');//오브젝트
			//alert(row);//Object....
